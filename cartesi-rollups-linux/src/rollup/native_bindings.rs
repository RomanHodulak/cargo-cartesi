...
pub fn rollup_read_advance_state_request(fd: c_int, finish: *mut rollup_finish, bytes: *mut rollup_bytes) -> Result<rollup_input_metadata, Box<dyn Error>> {
    unsafe {
...
pub fn rollup_read_inspect_state_request(fd: c_int, finish: *mut rollup_finish, query: *mut rollup_bytes) -> Result<(), Box<dyn Error>> {
    unsafe {
...
pub fn rollup_write_voucher(fd: c_int, address: [u8; 20], bytes: *mut rollup_bytes) -> Result<u64, Box<dyn Error>> {
    unsafe {
        let mut e = {
